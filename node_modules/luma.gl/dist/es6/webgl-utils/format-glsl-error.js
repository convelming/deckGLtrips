...
export function parseGLSLCompilerError(errLog, src, shaderType, shaderName) {
  ...
  const errors = {};
  const warnings = {}; // Patch the shader name

  const name = shaderName || getShaderName(src) || '(unnamed)';
  const shaderDescription = `${getShaderTypeName(shaderType)} shader ${name}`; // Parse the error - note: browser and driver dependent
