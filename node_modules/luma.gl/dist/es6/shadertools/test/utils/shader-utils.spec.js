...
test('shader-utils#getQualifierDetails', t => {
  ...
  QUALIFIER_TEST_CASES.forEach(testCase => {
    ...
    t.deepEqual(result, expected, `getQualifierDetails should return valid values when line=${line}`);
  });
  t.end();
});
...
void main() {
  ...
  PASSTHROUGH_TEST_CASES.forEach(testCase => {
    ...
    t.equal(result, expected, `Passthrough shader should match when version=${version} type=${inputType}`);
  });
  t.end();
});
